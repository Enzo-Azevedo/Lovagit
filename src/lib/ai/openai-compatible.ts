    ...
    async complete(request: CompletionRequest): Promise<CompletionResponse> {
      ...
      try {
        ...
        if (acc.text === '' || acc.toolCalls.size > 0) {
          ...
          // O que ja veio viaja com o erro. E' tudo token pago: descartar aqui
          // faz o reenvio comprar o mesmo trabalho de novo.
          const partial: PartialGeneration = {
            text: acc.text,
            reasoning: acc.reasoning,
            toolCalls: [...acc.toolCalls.values()].map((call) => ({
              name: call.name,
              partialArguments: call.args,
            })),
          };
          throw new ProviderError(
